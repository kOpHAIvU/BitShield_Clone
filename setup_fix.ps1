# PowerShell setup script for BitShield project - Fixed version
# Run this script in PowerShell with: .\setup_fix.ps1

param(
    [switch]$Force
)

Write-Host "Setting up BitShield project for Windows (Fixed version)..." -ForegroundColor Green
...
# Install pywin32 first to avoid conflicts
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to install dependencies. Trying alternative approach..." -ForegroundColor Yellow
    
    # Try installing without version constraints
    Write-Host "Installing dependencies without strict version constraints..." -ForegroundColor Yellow
    python -m pip install --no-deps -r requirements_windows.txt
