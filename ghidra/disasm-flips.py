...
def flip_and_disasm(offset, bitidx, first_addr, last_addr, complete_rest=False) -> List[Instruction]:
    ...
    # Disassembler will stop before normally unreachable instructions
    if complete_rest:
...
def get_disasm_info(offset, bitidx, no_restore=False) -> Union[DisasmInfo, None]:
    ...
    # If the user doesn't want to restore the bytes immediately, complete
    # disassembling other relevant bytes for inspection
    after_insts = flip_and_disasm(offset, bitidx, inst_first_addr, disasm_last_addr, complete_rest=no_restore)
...
if not isRunningHeadless():
    answer = askString(
        'Bit(s) to flip', '''Enter one of:
(a) a single bit index (0-7);
(b) an offset, bitidx pair: 0x123, 4;
(suffix R to disable immediately restoring flipped bytes); or
...
try:
    ...
    if answer.isnumeric():
        bitidx = int(answer)
        offset = utils.currentLocation().getByteAddress().getOffset()
        ...
        if answer.endswith('-bits.json'):
            ...
            disasm_infos = []
            for fname, f in loaded['fns'].items():
                # FIXME: Functions are not always continuous
                print(f'Flipping {fname}')
                base = f['base10_offset']
                disasm_infos.extend([get_disasm_info(base + byteoff, bitidx)
                                     for byteoff in range(f['size']) for bitidx in range(8)])
            outfile = f'./debug/all-fns-asm/{os.path.basename(answer).replace("-analysis.json", "-asm-all-fns.json")}'
