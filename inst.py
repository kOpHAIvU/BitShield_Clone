...
from utils import thread_first, warn
from cgutils import *
...
class CovModeConfig:
    ...
    type (a TensorType) of a layer (expr), returns the type of the coverage
    data for the layer.
    ...
    a list of layer coverage exprs and their types, returns: an expr for the
    overall coverage for the model and its type.
...
modecfg_rb = CovModeConfig('rb', lout2lcov_rb, lcov_type_xf=lcov_type_xf_rb, extra_params_fn=None)
...
modecfg_id = CovModeConfig('id', lout2lcov_identity, lcov_type_xf=lcov_type_xf_identity, extra_params_fn=None)
...
def lcov_type_xf_norm(t: r.TensorType):
    return r.TensorType((t.concrete_shape[1],), t.dtype)

def lout2lcov_l1(lout: r.Expr, shape, extra_param):
    return thread_first(lout, r.abs, r.sum, (r.reshape, (-1,)))

def lout2lcov_linf(lout: r.Expr, shape, extra_param):
    return thread_first(lout, r.abs, r.max, (r.reshape, (-1,)))

def lcovs2ocov_l1l2(lcovs, types):
    return r.reshape(reduce(r.add, lcovs), (-1,)), r.TensorType((1,))

def lcovs2ocov_linf(lcovs, types):
    return r.reshape(reduce(r.maximum, lcovs), (-1,)), r.TensorType((1,))

modecfg_l1 = CovModeConfig(
    'l1', lout2lcov_l1, lcov_type_xf=lcov_type_xf_norm,
    extra_params_fn=None, lcovs2ocov=lcovs2ocov_l1l2
)

modecfg_linf = CovModeConfig(
    'linf', lout2lcov_linf, lcov_type_xf=lcov_type_xf_norm,
    extra_params_fn=None, lcovs2ocov=lcovs2ocov_linf
)
...
delegated_modes = set(
    k for k, v in cov_mode_configs.items() if v.lout2lcov is None
)
...
def instrument_module_gn(
    ...
    verbose=False, coop_cig_magic_float=None
):
    ...
    def ftransform(func: r.Function, mod: IRModule, ctx: PassContext):
        ...
        if coop_cig_magic_float is not None:
            grads_out_map[model_out][-1] *= r.const(coop_cig_magic_float)
