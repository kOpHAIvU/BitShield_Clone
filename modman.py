...
def load_bounds(model_name, dataset, mode, data_path=None) -> list:
    if not data_path:
        data_path = f'{cfg.coverages_dir}/{dataset}-{model_name}-{mode}.pth'
...
def create_gemod(gefmod, debug=False, debug_dump_root=None):
    '''Creates a Graph Executor module from a Graph executor factory module.'''
    dev = tvm.device('cpu')
    if not debug:
        return graph_executor.GraphModule(gefmod["default"](dev))
    return debug_executor.create(gefmod['get_graph_json'](), gefmod, dev, dump_root=debug_dump_root)
...
def get_torch_mod(model_name, dataset):
    ...
    if 'state_dict' in maybe_state_dict:  # Trained by pytorch-train-imagenet.py
        state_dict = maybe_state_dict['state_dict']
...
def get_irmod(
    ...
    if mode not in [
        'nd', 'id', 'rb', 'cb', 'npcb', 'gn1', 'gn2', 'gninf', 'l1', 'linf'
    ] and include_extra_params:
...
def get_irmod_by_bi(bi: utils.BinaryInfo, **kwargs):
    return get_irmod(
        bi.model_name, bi.dataset, bi.dig, bi.batch_size, bi.input_img_size,
        nchannels=bi.nchans, **kwargs
    )

def load_module(path, debug=False, debug_dump_root=None):
    libmod: tvm.runtime.Module = tvm.runtime.load_module(path)
    return create_gemod(libmod, debug=debug, debug_dump_root=debug_dump_root)

def load_built_model(bi: utils.BinaryInfo, fpath=None):
    fpath = fpath or bi.fpath
    if bi.compiler == 'tvm':
        return WrappedRtMod(load_module(fpath), output_defs=bi.output_defs, batch_size=bi.batch_size, nclasses=bi.nclasses)
    elif bi.compiler == 'glow':
        return GlowModel(fpath, output_defs=bi.output_defs, batch_size=bi.batch_size, nclasses=bi.nclasses)
    else:
        raise ValueError(f'Unknown compiler: {bi.compiler}')

def build_module(
    irmod, params,
    export_path=None, opt_level=3, target=targets['avx2'],
    is_qnn=False, cig_make_space=True, exportfn=None
):
    pctx_cfg = {
        'tir.disable_assert': True,
        **tvm.transform.PassContext.current().config
    }
    if cig_make_space:
        tir_lower_passes = pctx_cfg.get('tir.add_lower_pass', [])
        tir_lower_passes.append((3, cig.SpacemakerPass()))
        pctx_cfg['tir.add_lower_pass'] = tir_lower_passes
    ...
    if is_qnn:
        ...
        with tvm.transform.PassContext(opt_level=0, config=pctx_cfg):
            gefmod = relay.build(irmod, target=target, params=params)
    else:
        with tvm.transform.PassContext(opt_level=opt_level, config=pctx_cfg):
            gefmod = relay.build(irmod, target=target, params=params)
    print(f'Module built in {time.time() - start_time:2f} seconds.')
    if export_path:
        utils.ensure_dir_of(export_path)
        if exportfn:
            exportfn(gefmod, export_path)
            # The user may have added something to the exported module, so we
            # reload it instead of using the one returned by relay.build
            gefmod = tvm.runtime.load_module(export_path)
        else:
            gefmod.export_library(export_path)
        print(f'Saved to {export_path}.')
    gemod = create_gemod(gefmod, debug=False)
    return gemod, gefmod
