...
def attack_with_dig_protection(model_name, dataset_name, device='cpu', attack_mode='noise', attack_iters=25):
    ...
    if getattr(attack_with_dig_protection, "_obfus_sig_cfg", None) is not None:
        ...
        obfus_runtime = ObfusSigRuntime(
            ...
            device=device,
            obfus_targets=cfg_os.get("obfus_targets", ("linear",)),
            max_obfus_layers=cfg_os.get("max_obfus_layers", None),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--obfus-max-layers', type=int, default=None, help='Maximum number of layers to wrap (None/0 = all layers, N = wrap N layers)')
    ...
    if args.obfus_sig:
        # Parse obfus_targets from comma-separated string
        obfus_targets_list = [t.strip() for t in args.obfus_targets.split(',') if t.strip()]
        # Handle max_obfus_layers: None/0 means all layers
        max_obfus_layers = None if args.obfus_max_layers is None or args.obfus_max_layers == 0 else args.obfus_max_layers
        attack_with_dig_protection._obfus_sig_cfg = {
            ...
            "obfus_targets": obfus_targets_list,
            "max_obfus_layers": max_obfus_layers,
            "initial_reseed": bool(args.obfus_initial_reseed),
            "proactive_period": args.obfus_auto_reseed,
