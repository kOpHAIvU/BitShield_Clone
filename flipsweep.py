...
class FlipResult:
    ...
    @staticmethod
    def empty():
        return FlipResult(-1., None, -1., -1., -1., 0.)
    ...
    def get_completed_or_new(cls, args, abs_byteoff, bitidx, bi_idx):
        ...
        return cls(
            ...
            getattr(retcoll, 'sus_scores', [0.]*nbis)[bi_idx],
            from_completed=True,
        )
...
@dataclass
class FlipResultCollV3(FlipResultCollV2):
    sus_scores: List[float]

    @staticmethod
    def empty(base10_offset, bitidx):
        return FlipResultCollV3(base10_offset, bitidx, [], [], [], [], [], [])

    def append_result(self, ret: FlipResult):
        super().append_result(ret)
        self.sus_scores.append(ret.sus_score)
...
class SweepResultV3:
    ...
    retcoll_map: Dict[
        int,
        List[Union[FlipResultCollV1, FlipResultCollV2, FlipResultCollV3]]
...
completing_retcoll_map: Dict[int, List[Union[FlipResultCollV1, FlipResultCollV2, FlipResultCollV3]]] = {}
...
def load_sweep_result(fname):
    with open(fname, 'rb') as f:
        ...
        # Migrate old args
        if 'cig' not in ret.args:
            ret.args.cig = ret.args.cfig
        if 'dig' not in ret.args:
            ret.args.dig = ret.args.dfig
        return ret
...
def get_val_loader(bi, fast, mc_nsamples):
    if mc_nsamples:
        ...
        return dataman.get_sampling_loader_v2(bi.dataset, bi.input_img_size, 'test', cfg.batch_size, n_per_class=bi.fast_n_per_class, num_workers=0)
    return dataman.get_benign_loader(bi.dataset, bi.input_img_size, 'test', cfg.batch_size, num_workers=0)

def flip_proc_work(bi: utils.BinaryInfo, byteidx, bitidx, is_monte_carlo, is_gan, gan_images_only, fret):
    """The *real* work done by the child process that loads the model and does the flip."""
    ...
    if is_gan:
        ...
        if is_monte_carlo:
            val_loader.sampler.generator.manual_seed((byteidx << 8) | bitidx)
        if bi.has_dig:
        ...
        fret.extend([acc, top_labels, sus_score])
...
def flip_thread_work(next_byte_fn, on_byte_done, bis, args):
    ...
    while True:
        ...
        for bitidx in range(8):
            ...
            frc = FlipResultCollV3.empty(abs_byteoff, bitidx)
            for i, bi in enumerate(bis):
                byteidx = abs_byteoff - cfg.GHIDRA_BASE_ADDR - lmis[bi].region_elf_offset
                ...
                if not fr.from_completed:
                    ...
                    proc = mp.Process(target=flip_proc_work, args=(
                        bi, byteidx, bitidx, bool(args.monte_carlo_nsamples), args.gan, args.gan_images_only,
                        fret
                    ))
                    proc.start()
                    ...
                    if proc.exitcode != 0:
                        ...
                        break
                    if len(fret) != (1 if args.gan else 3):
                        unflippable_reason = f'🤯 len(fret) {len(fret)}'
                        break

                    if not args.gan:
                        acc, fr.top_labels, fr.sus_score = fret
                ...
                if not args.gan:
                    print(f'Acc. after flipping: {fr.correct_pct:.2f}%, sus: {fr.sus_score:.2f}')
                else:
...
def get_all_bytes(fns, nchunks=None, chunk_idx=None):
    # Generates Ghidra offsets for fns
    ret = []
    for _fname, f in fns.items():
        fn_start = f['base10_offset']
        nbytes = f['size']
...
def read_existing_sweepret(path, expected_args):
    ...
    ret.args.min_timeout_secs = expected_args.min_timeout_secs
...
def sweep(args):
    ...
    bis = (
        ...
        .map(lambda d: utils.BinaryInfo(
            args.compiler, args.compiler_version, args.model_name, d,
            args.cig, args.dig,
            avx=not args.no_avx, opt_level=args.opt_level
        ))
        .list()
    )
    fns = (
    ...
    assert fns is not False, 'All function analysis data must be the same'

    if not args.specific_bits_file:
        all_bytes = get_all_bytes(fns, nchunks=args.nchunks, chunk_idx=args.chunk_idx)
        total_bytes = len(all_bytes)
        real_total_bytes = len(get_all_bytes(fns))
    else:
    ...
    if os.path.exists(outfile):
        ...
        old_progress = len(oldret.retcoll_map) / real_total_bytes
        assert old_progress <= 1, f'Old progress is {old_progress:.2%} (> 100%)'
        print(f'Old progress: {len(oldret.retcoll_map)}/{real_total_bytes} bytes ({old_progress:.2%})')
    ...
    if args.completing_file:
        ...
        for key in [
            'compiler', 'compiler_version', 'model_name', 'cig', 'dig',
            'opt_level', 'no_avx'
        ]:
            assert getattr(com.args, key) == getattr(args, key)
    ...
    if not args.gan:
        val_loaders.update({
            bi: get_val_loader(bi, args.fast, args.monte_carlo_nsamples) for bi in tqdm(bis, desc='Loading val loaders')
        })
        accs = []
        for bi in bis:
            start_time = time.time()
    ...
    if args.timeout == 0:
        args.timeout = max(baseline_eval_time * args.timeout_x, args.min_timeout_secs)
    ...
    print(f'Finished sweeping {outfile}.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-F', '--no-fast', action='store_false', dest='fast', help="Don't use fewer images for faster testing")
    parser.add_argument('-N', '--monte-carlo-nsamples', help="Number of samples for Monte Carlo estimation of accuracy (0 to disable)", type=int, default=0)
    ...
    parser.add_argument('-t', '--timeout-x', type=float, default=0, help='Timeout multiplier (0 for auto)')
    parser.add_argument('-T', '--timeout', type=float, default=0, help='Timeout in seconds (0 for auto)')
    parser.add_argument('--min-timeout-secs', type=float, default=1, help='Minimum timeout in seconds when timeout is automatically determined')
    ...
    parser.add_argument('-o', '--out-dir', type=str, default=cfg.sweep_dir)
