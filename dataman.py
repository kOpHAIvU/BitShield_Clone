...
def ImageNetDataset(image_size, normalize=True, split='train'):
    return datasets.ImageNet(
        root=cfg.imagenet_root,
        split='train' if split == 'train' else 'val',
        transform=transforms.Compose([
            transforms.Resize(int(image_size / (224/256))),
            transforms.CenterCrop(image_size),
            transforms.ToTensor(),
        ] + ([
...
class LegacyImageNetDataset(Dataset):
    def __init__(self,
                 image_size,
                 image_dir=f'{cfg.imagenet_root}/',
                 label2index_file=f'{cfg.imagenet_root}/ImageNetLabel2Index.json',
                 split='train'):
        super(LegacyImageNetDataset).__init__()
        split = 'train' if split == 'train' else 'val'
        self.image_dir = image_dir + split + ('/' if len(split) else '')
        self.transform = transforms.Compose([
            transforms.Resize(image_size),
        ...
        for cls in self.classes:
            imgs_in_cls = sorted(os.listdir(self.image_dir + cls))
            self.image_list += [self.image_dir + cls + '/' + image_name for image_name in imgs_in_cls]
...
def FakeDataset(image_size, set_id=0, split='train', nimages=10000, nclasses=10, nchannels=3):
    ...
    return datasets.FakeData(
        ...
        transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((4.9930,), (2.9030,))
        ]))
...
def make_loader(dataset, batch_size, num_workers=4, size_limit=0, dataset_handler=None, shuffle=False, **kwargs):
...
def get_sampling_benign_loader(dataset_name, image_size, split, batch_size, frac_per_class, start_frac=0., **kwargs):
    ...
    nclasses, nimgs_per_class = {
        ...
        # TODO: Support other datasets (e.g., ImageNet) (fn currently unused for debfd)
    }[dataset_name]
...
def get_sampling_loader_v2(dataset_name, image_size, split, batch_size, n_per_class=10, shift=0, allow_insufficient=False, **kwargs):
    dataset = get_benign_dataset(dataset_name, image_size, split)
    iterable = dataset
    # Accelerate if possible (e.g., for ImageNet)
    if isinstance(dataset, datasets.DatasetFolder):
        iterable = dataset.samples
    indices_by_cls = {}
    shifts_by_cls = {}
    for idx, (_, y) in enumerate(iterable):
...
def get_monte_carlo_loader(dataset_name, image_size, split, batch_size, n=100, init_seed=42, **kwargs):
    '''A data loader that randomly returns n samples from the dataset.
    For reproducibility, manually reseed each time before using via loader.sampler.generator.manual_seed(int).'''
    dataset = get_benign_dataset(dataset_name, image_size, split)
    generator = torch.Generator()
    generator.manual_seed(init_seed)
    sampler = RandomSampler(dataset, num_samples=n, generator=generator)
    return make_loader(dataset, batch_size, sampler=sampler, **kwargs)

def get_per_cls_idxs(dataset):
    indices_by_cls = {}
