...
def type_inferred(expr, inplace=False):
    ...
    try:
        ...
        expr_str = str(expr)
        print(expr_str, end='', file=sys.stderr)
...
def DumpTIRPass(outfile=None, at_opt_level=0):

    outfile_ready = False

    @tvm.tir.transform.prim_func_pass(opt_level=at_opt_level)
    def dump_tir(fn, mod, ctx):
        nonlocal outfile_ready
        if not outfile_ready:
            if outfile is not None:
                # Create parent dir and truncate file
                utils.ensure_dir_of(outfile)
                with open(outfile, 'w') as f:
                    pass
            outfile_ready = True
